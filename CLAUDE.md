...
Git identity + auth wiring spans the Containerfile and `run.sh`. On login, `/etc/profile.d/gitconfig-init.sh` (written during the image build) generates `~/.gitconfig` if one doesn't already exist in the volume: it `[include]`s the read-only host `.gitconfig.host` , overrides the credential helper to `!gh auth git-credential`, and forces LF line endings (`core.autocrlf = false`, `core.eol = lf`) regardless of the host setting. The effect is that `git push` inside the container reuses the host `gh` token (via the read-only `~/.config/gh` mount) without needing a separate credential setup. If the wiring looks wrong, deleting `~/.gitconfig` from the `ccontainer-home` volume will cause it to be regenerated on next login.
